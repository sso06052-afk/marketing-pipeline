...
> Git은 코드를 받고 업데이트하는 도구입니다. 한 번만 설치해두면 추후 업데이트도 편해집니다.

## 1-1. Git 설치

### A. Mac

대부분의 Mac은 Git이 이미 설치되어 있습니다. 확인부터 해볼게요.
...
2. 검은 창에 다음 입력:
   ...
   ```
3. 결과:
   - `git version 2.x.x` 같이 버전이 나오면 ✅ 설치됨, 1-2로 이동
   - 설치 안내 창이 뜨면 **Install** 클릭 → 5분 정도 자동 설치됨

### B. Windows

1. 브라우저에서 **`git-scm.com/download/win`** 접속
2. **64-bit Git for Windows Setup** 클릭하여 다운로드
3. 다운받은 `.exe` 파일 더블클릭
4. 설치 화면에서 **모든 옵션 기본값 그대로 두고 Next 계속 클릭**
5. 마지막 **Install** 클릭 → 약 1분 후 완료
...
2. 다음 입력:
   ...
   git --version
   ```
3. `git version 2.x.x` 나오면 ✅ 완료

## 1-2. 메모장 띄워두기

키값을 정리할 메모장을 미리 열어둡니다. (닫지 말고 계속 켜두세요)
...
이미 열려있는 터미널/PowerShell에서 진행합니다.
...
**확인:** 바탕화면에 **`marketing-pipeline`** 폴더가 생겼는지 확인.

> 폴더 이름이 `marketing-pipeline` (하이픈) 입니다. `_` 아닙니다.
...
3. 받으신 폴더에서 **`marketing-pipeline/supabase/schema.sql`** 파일 열기
...
8. 마이그레이션 파일도 추가 실행:
   - **+ New query** 다시 클릭
   - **`marketing-pipeline/supabase/migrations/20260429_add_deal_columns.sql`** 파일을 같은 방법으로 복사·붙여넣기·Run
...
5. **Cmd + S** 로 저장창 띄우기
6. 저장 위치: **바탕화면 → marketing-pipeline 폴더**
...
4. **Ctrl + S** 로 저장창 띄우기
5. 저장 위치: **바탕화면 → marketing-pipeline 폴더**
...
`marketing-pipeline` 폴더 안에 **`.env`** 파일이 만들어졌는지 확인.
...
1. **Cmd + Space** → "터미널" 입력 → 엔터 (이미 열려있으면 그대로 사용)
2. 다음 명령어 입력:
...
3. 확인: **`ls`** 입력 후 엔터 → **`Dockerfile`** 글자가 보이면 OK

## 5-1-B. Windows

1. 시작 메뉴 → "PowerShell" 검색 → 실행 (이미 열려있으면 그대로 사용)
2. 다음 명령어 입력:
...
- [ ] Git 설치 완료 (`git --version` 실행됨)
- [ ] `marketing-pipeline` 폴더 받음
- [ ] API 키 6개 메모장에 정리
- [ ] Docker Desktop 실행 중 (🐳 아이콘)
- [ ] `.env` 파일이 marketing-pipeline 폴더 안에 존재
...
새 버전이 나왔을 때 사용하세요. **명령어 한 줄이면 끝납니다.**
...
→ 5~10분 기다리면 끝. 화면에 **`✅ 업데이트 완료!`** 나오면 성공.
...
```bash
cd ~/Desktop/marketing-pipeline   # Windows: cd $HOME\Desktop\marketing-pipeline
git pull
docker rm -f pipeline-dashboard
docker build --build-arg NEXT_PUBLIC_SUPABASE_URL=... --build-arg NEXT_PUBLIC_SUPABASE_ANON_KEY=... -t marketing-pipeline .
docker run -d --name pipeline-dashboard -p 3000:3000 -v $(pwd)/.env:/app/.env --restart unless-stopped marketing-pipeline
```
